...
public class SvnUtil {
  ...
  protected static final String COMMIT_MESSAGE_MERGE_PREVIOUS =
      "r%1$s: [%2$s] (%3$tY-%3$tm-%3$td %3$tH:%3$tM:%3$tS) %4$s\n"; //$NON-NLS-1$

  /**
   * Builds a working copy of the target branch of the given mergeUnit. This working copy consists at least of the given
   * paths and is most likely not a whole checkout.
   * 
   * @param mergeUnit
  ...
  /**
   * Sometimes a TortoiseSVN process keeps holding handles of SVN repositories. To delete a repository we try to kill
   * this process. Killing this process is considered safe.
   * 
   * @throws SvnUtilException
   */
  ...
  private static boolean setupWorkingCopy(SVNMergeUnit mergeUnit, ISvnClient client) throws SvnUtilException {
    ...
    if (!missingFiles.isEmpty()) {
      ...
      String dialogMessageScrollable = getDialogScrollableMessage(joinedMissingFiles, workingFolderManual, mergeUnit);
      ...
      final MessageDialog dialog = new MessageDialogScrollable(ApplicationUtil.getApplicationShell(),
          Messages.SvnUtilSvnkit_MovedFiles_Title, null, dialogMessage, dialogMessageScrollable, MessageDialog.WARNING,
          new String[] {Messages.SvnUtilSvnkit_MovedFiles_CancelMerge,
              Messages.SvnUtilSvnkit_MovedFiles_ContinueWithoutMergingMissingFiles},
          0);

      final AtomicInteger choice = new AtomicInteger();
      E4CompatibilityUtil.getApplicationContext().get(UISynchronize.class).syncExec(() -> choice.set(dialog.open()));
   ...
   * 
   * @param joinedMissingFiles all the file names joined.
   * @param workingFolderManual path of the working folder.
   * @param mergeUnit {link SVNMergeUnit} .
  ...
  private static String getDialogScrollableMessage(String joinedMissingFiles, String workingFolderManual,
    ...
    String commandMerge =
        String.format("SVN merge -r %d:%d %s <PATH_OF_MOVED_FILE_IN_SOURCE_BRANCH> <FILENAME> --accept postpone %n", //$NON-NLS-1$
   ...
   * @throws SvnUtilException
   */
  private static void mergeChangesSeparately(SVNMergeUnit mergeUnit, final ISvnClient client) throws SvnUtilException {
    LogUtil.entering(mergeUnit);
   ...
   * 
   * @param url
   * @return the repository root of the url. when no glues for the root have been found the url itself will be return
   *         because it is expected that the url is already the root.
   */
