...
public class SvnClientJavaHl extends AbstractSvnClient {
	...
	public long[] updateEmpty(List<Path> paths) throws SvnClientException {
		...
		for (int i = 0; i < paths.size(); i++) {
			try {
				/*
				 * setDepth = false : Otherwise the depth is set to the local checked out
				 * repository and files are deleted existing in the directory. We don't want to
				 * modify the checked out hierarchy, only update the file.
				 */
	...
	public List<String> getConflicts(Path path) throws SvnClientException {
		try {
			...
			return Arrays.stream(statusArray) //
					.filter(status -> status.hasTreeConflict() || status.getConflictWorking() != null
							|| status.getTextStatus() == SVNStatusKind.CONFLICTED) // only conflicts of interest
