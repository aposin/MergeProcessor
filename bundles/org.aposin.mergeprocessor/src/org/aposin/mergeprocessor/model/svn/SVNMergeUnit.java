...
public final class SVNMergeUnit implements IMergeUnit, PropertyChangeListener {
    ...
    public Path convertSvnDiffToPath(final SvnDiff diff) {
        /*
         * diff.getUrl().toURI() cannot be used as it is not able to deal
         * with spaces. So we use simple String replacement to achieve
         * the same functionality
         */
        if (diff.getUrl().toString().startsWith(getUrlSource())) {
            return Paths.get(diff.getUrl().toString().substring(getUrlSource().length() + 1)); //+1 to escape the leading slash
        } else {
